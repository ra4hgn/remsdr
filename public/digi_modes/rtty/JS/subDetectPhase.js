// Bit Detection on phase comparaison at the output of the bandpass filters
const DetectPhase = {
    On: true,
    Disp: "displayPhi",
    Text: "textPhi",
    ...
    DetectOn: function () {
        this.On = !this.On;
        $("#RXzonePhi").css("display", this.On ? "block" : "none");
    },
    Detect: function () {
        if (this.On) {
            var FilterLength = modes[RTTYstore.idxMode].Nstep;
            var ShortLength = modes[RTTYstore.idxMode].Nsample;
            var Fmean = (RTTY_RX.Fmark + RTTY_RX.Fspace) * FilterLength * ShortLength / 10000;
            ...
            while (this.IdxInt != IdxLoadPF) {
                ...
                var FiltreOut = 0
                    for (var i = 1; i <= FilterLength; i++) {
                        ...
                        FiltreOut += ShortInt.Phase[j]; //Long integration number of transition
                    }
                    FiltreOut = FiltreOut - Fmean
            ...
            while (Idx != this.IdxInt) {
                ...
                var out = (Fo > 0.9 * this.MaxPlot) ? true : false;
    ...
    DispPlot: function () {
        this.Plot = !this.Plot;
        $("#displayPhi").css("display", (this.Plot) ? "block" : "none");
        $(".text_env_P").css("height", (this.Plot) ? "120px" : "170px");
    },
    Resize: function () {
